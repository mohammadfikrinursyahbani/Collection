...
st.title('Collection Dashboard :sparkles:')
...
with col2:
    total_revenue = format_currency(
        daily_order_df.revenue.sum(), 'AUD', locale='es_CO')
...
ax[0].set_title('Best Performing Products', fontsize=50)
...
ax[1].set_title('Worst Performing Products', fontsize=50)
...
sns.barplot(y='frequency', x='customer_id', data=rfm_df.sort_values(
    by='frequency', ascending=False).head(5), ax=ax[1], palette=colors)
ax[1].set_ylabel(None)
...
sns.barplot(y='monetary', x='customer_id', data=rfm_df.sort_values(
    by='monetary', ascending=False).head(5), ax=ax[2], palette=colors)
...
st.caption('Created by: Mohammad Fikri Nur Syahbani')
